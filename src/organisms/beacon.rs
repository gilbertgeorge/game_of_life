...
pub fn generate(board: &mut Board, position: (usize, usize)) {
    ...
    board.set_cell((row + 1) % board.height, (column + 1) % board.width, Cell::Alive);
    board.set_cell((row + 2) % board.height, (column + 2) % board.width, Cell::Alive);
    board.set_cell((row + 2) % board.height, (column + 3) % board.width, Cell::Alive);
    board.set_cell((row + 3) % board.height, (column + 2) % board.width, Cell::Alive);
